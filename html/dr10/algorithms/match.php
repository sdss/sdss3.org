...
<code>bestObjID</code>, and are most appropriate for spectrophotometry
and for sources known to be stars. <code>bestObjID</code> is the
"default" value.  Flux-based matches are stored as
<code>fluxObjID</code>, and can be superior if you primarily want
redshifts of galaxies; in particular, they recover a number of nearby
galaxy much better than the position-based match.</p>
...
are best; that is, in such cases, the flux-based match is unlikely to
be appropriate and in some cases can be incorrect (for example M stars
in close pairs with bluer stars). However, many in such cases, if the
position-based match and flux-based match disagree
(<code>fluxObjID!=bestObjID</code>) than the photometry and astrometry
of the position-based match are incorrect and should be used with great
caution.</p>

<h2 id="position">Position-based matches</h2>

<p>Position-based matches are the default matches. They are
particularly important to use if you are primarily interested in the
<a href="dr10/algorithms/magnitudes.php">fiber fluxes</a>, for example
for spectrophotometry. In addition, if you are
<strong>certain</strong> that your sources of interest are point
sources, the position-based match is virtually always the right
choice.</p>
...
photometric object is stored in <code>bestObjID</code> in the specObj
tables.</p>
...
href="dr10/help/glossary.php#parent">parent</a> object) then the object is
...
<code>fluxObjID</code> associated with the spectrum. </li>
...
the photometry is irretrievably bad in some way (such as a long
...
stored in the <a
href="http://data.sdss3.org/datamodel/files/SPECTRO_REDUX/photoMatchPlate.html">photoMatchPlate</a>
file (the columns with the "flux" moniker), and the detailed
...
matched photometric object is stored in <code>fluxObjID</code> in the
specObj tables.</p>

<h2 id="example">Examples of flux-based matching</h2>
...
<p>Occasionally flux-based matching fails in cases where the
photometry is actually correct.  Typically this occurs for sources of
unusual color that have close neighbors.</p>
